...
impl Agent {
    ...
    #[instrument(skip(self), fields(tool_name = ?tool_call))]
    ...
    async fn get_systems_resources(&self) -> AgentResult<HashMap<String, HashMap<String, (Resource,String)>>> {
        ...
        for system in &self.systems {
            debug!(system_name = system.name(), "Getting system status");
    ...
    async fn prepare_inference(
        ...
        debug!(message_count = new_messages.len(), "Prepared inference messages");
        Ok(new_messages)
    }
    ...
    pub async fn reply(&self, messages: &[Message]) -> Result<BoxStream<'_, Result<Message>>> {
        ...
        messages = self.prepare_inference(&system_prompt, &tools, &messages, &Vec::new(), ESTIMATED_TOKEN_LIMIT)
            .instrument(tracing::info_span!("prepare_initial_inference"))
            .await?;
        ...
        Ok(Box::pin(async_stream::try_stream! {
            loop {
                ...
                let (response, _) = self.provider.complete(
                    ...
                    &messages,
                    &tools,
                ).instrument(response_span).await?;

                debug!("Got completion response");
                ...
                debug!("Found {} tool requests", request_count);

                if tool_requests.is_empty() {
                    info!("No more tool requests, ending reply");
                    break;
                }

                // Then dispatch each in parallel
                let tool_span = tracing::info_span!("tool_dispatch", request_count);
                let futures: Vec<_> = tool_requests
                    .iter()
                    .map(|request| {
                        let req_span = tracing::debug_span!("tool_request", 
                                        tool = request.tool_call.as_ref()
                                            .map(|t| t.name.as_str())
                ...
                messages = self.prepare_inference(&system_prompt, &tools, &messages, &pending, ESTIMATED_TOKEN_LIMIT)
                    .instrument(tracing::info_span!("prepare_next_inference"))
                    .await?;
            }
        }))
    }
